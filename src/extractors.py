...
- Añadido Type Hinting avanzado (TypedDict)
"""
from __future__ import annotations
import logging
import re
from typing import List, Dict, Any, Tuple, TypedDict # <-- MODIFICADO
...
# --- INICIO DE NUEVA DEFINICIÓN DE TIPO ---
...
# --- FIN DE NUEVA DEFINICIÓN DE TIPO ---
...
# ===== Parse block =====
def _parse_block(block: str, fecha: str, hora: str | None) -> ExtractedRow | None: # <-- MODIFICADO
# ... (el resto de esta función no cambia) ...
    b = re.sub(r"(\d)\s*\n\s*(\d)", r"\1\2", block)
    b = normalize_space(b)
    tokens = _tokens(b)
    ...
    # MODIFICADO: Añadimos el "type hint" a la variable row
    row: ExtractedRow = {
...
def parse_pdf_text(pdf_path: str | Path) -> Tuple[List[ExtractedRow], List[int]]: # <-- MODIFICADO
    rows: List[ExtractedRow] = []; by_page: List[int] = [] # <-- MODIFICADO
...
def parse_pdf_tabula(pdf_path: str | Path) -> Tuple[List[ExtractedRow], List[int]]: # <-- MODIFICADO
# ... (el resto de esta función no cambia) ...
    ...
    rows: List[ExtractedRow] = [] # <-- MODIFICADO
    for df in dfs:
...
def parse_pdf_ocr(pdf_path: str | Path) -> Tuple[List[ExtractedRow], List[int]]: # <-- MODIFICADO
# ... (el resto de esta función no cambia) ...
...
def parse_pdf_any(pdf_path: str | Path, use_ocr: bool = False) -> Tuple[List[ExtractedRow], List[int], str]: # <-- MODIFICADO
